...
from leavestracker.apps.employees.serializers import CustomUserSerializer
    ...
    def post(self,request):
        ...
        try:
            ...
            if str(exc)==const.USERNAME_VALID_ERROR_MSG:
                queryset = CustomUser.objects.filter(username=serializer.data['username']).first()
                (data, response_status) = (CustomUserSerializer(instance=queryset).data, status.HTTP_200_OK)
            else:
